...
object Main extends App {
  ...
  println("The most viewed page in Spain is ", mostViewedRDD)
  ...
  println("Top ten countries visiting Wikimedia")
  topTenRDD.foreach(println)
  val codeMap = Source.fromFile("country-codes.csv").getLines().map(_.toLowerCase.split(",").take(2)).map {
    case Array(name, code)  => (code, name)
  }.toMap
  println("writing to visits file")
  visitsPerCountry(codeMap, views, "visits.csv")
  val count = countNoVisits(views)
